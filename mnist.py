...
for e in range(epochs):
    ...
    for idx_p, pbag in enumerate(pos_bags):
        pbag=pbag.float()
        pbag=Variable(pbag).type(torch.FloatTensor)
        ...
        for idx_n, nbag in enumerate(neg_bags):
            nbag=nbag.float()
            nbag=Variable(nbag).type(torch.FloatTensor)
            n_scores=cnn.forward(nbag[0])

            max_n=torch.max(n_scores)
            z=np.array([0.0])
            loss+=torch.max(Variable(torch.from_numpy(z)).type(torch.FloatTensor), (max_n-max_p+1))
#            loss=torch.max(torch.tensor(0.0), (max_n-max_p+1))
    ...
    for batch_idx, (data, label) in enumerate(val_loader):
        ...
        valbag=data.float()
        valbag=Variable(valbag).type(torch.FloatTensor)
...
for batch_idx, (data, label) in enumerate(test_loader):
    ...
    tsbag=data.float()
    tsbag=Variable(tsbag).type(torch.FloatTensor)
